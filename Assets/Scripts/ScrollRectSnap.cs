...
public class ScrollRectSnap : MonoBehaviour
{
    ...
    void Update()
    {
        if (LerpPaged)
        {
            ...
            scroll.horizontalNormalizedPosition = Mathf.Lerp(scroll.horizontalNormalizedPosition, points[currentPage], (snapSpeed/2) * Time.deltaTime);
            if (Mathf.Approximately(scroll.horizontalNormalizedPosition, points[currentPage])) LerpPaged = false;
        }
    ...
    public void PagedRight()
    {
        if (currentPage != screens-1)
        {
            currentPage++;
            LerpPaged = true;
        }
    }
    public void PagedLeft()
    {
        if (currentPage != 0)
        {
            currentPage--;
